 ...
 *
 * 子命令：ailo-desktop init [--defaults] — 初始化 .env 与 Skills 目录（见 cli.ts）。
 */
...
/** 默认使用本地蓝图路径；可通过 .env 的 BLUEPRINT_DESKTOP_URL 覆写 */
const BLUEPRINT_URL =
...
async function main(): Promise<void> {
  ...
  const connectionState = {
    connected: false,
    endpointId: "",
    displayName: process.env.DISPLAY_NAME ?? "桌面Agent",
  };
  let webchatCtxRef: EndpointContext | null = null;
  let connectAttempt = 0;
  /** 未连接时防止重复 runEndpoint（连续保存只保留一次连接尝试） */
  let endpointConnecting = false;

  /** 用最新配置发起或重连；overrides 来自配置页保存时传入 */
  async function applyConnectionConfig(overrides?: AiloConnectionConfig): Promise<void> {
    const cfg = overrides ?? loadConnectionConfig(envFilePath);
    ...
    try {
      ...
      runEndpoint({
        ...
        onConnectFailure: async (err, client) => {
          ...
          const latest = loadConnectionConfig(envFilePath);
          ...
          try {
            ...
            // 不 rethrow，让 bootstrap 继续循环尝试 connect()
          }
        },
      });
    } catch (e) {
  ...
  const initial = loadConnectionConfig(envFilePath);
